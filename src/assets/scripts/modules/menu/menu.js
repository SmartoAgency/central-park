...
function openMenu(button, menu) {
    ...
    menuLinks.forEach(el => {
        el.addEventListener('click', () => {
            console.log('clicki');
            document.querySelectorAll('[data-menu-close]')[0].click();
        })
    })
    button.addEventListener('click', function(evt) {
        gsap.timeline({
            ...
            .fromTo(menuLinks, { y: 50, autoAlpha: 0, }, { y: 0, autoAlpha: 1, clearProps: 'all', duration: 1.25 },
                '<')
            .add(() => {
...
function closeMenu(button, menu) {
    ...
    button.addEventListener('click', function(evt) {
        ...
        .add(enableCurtainFromTop(), '<')
            ...
            .fromTo(menuLinks, { y: 0, autoAlpha: 1, }, { y: 50, autoAlpha: 0, duration: 0.75 },
                '<-0.55')
            .add(() => {
